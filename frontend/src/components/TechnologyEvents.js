...
import "../style/TechnologyEvents.css";
...
function TechnologyEvents() {
  ...
  const fetchRecommendations = async (append = false) => {
    try {
      ...
      // ✅ Define keyword array (only for technology)
      const techKeywords = [
        "tech",
        "technology",
        "AI",
      ...
      // ✅ Correct: no stray 'keywords' reference
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/recommendations?keywords=${encodeURIComponent(
          JSON.stringify(techKeywords)
      ...
      setRecommendations((prev) => {
        const existingKeys = new Set(
          prev.map((e) => `${e.id}-${e.title}`.toLowerCase())
        );
  ...
  return (
    <div className="home-page">
      {/* Title */}
      <h2 className="title">Technology Event Recommendations</h2>
